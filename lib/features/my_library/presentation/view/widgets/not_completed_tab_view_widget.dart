...
class NotCompletedTabViewWidget extends StatefulWidget {
  final MyLibraryCubit myLibraryCubit;
  final HomeCubit homeCubit;
  const NotCompletedTabViewWidget({
    super.key,
  ...
  @override
  State<NotCompletedTabViewWidget> createState() =>
      _NotCompletedTabViewWidgetState();
}

class _NotCompletedTabViewWidgetState extends State<NotCompletedTabViewWidget> {
  @override
  void initState() {
    super.initState();
      ...
      body: BlocBuilder<MyLibraryCubit, MyLibraryState>(
        ...
        builder: (context, state) {
          if (state is GetAllAudioFilesFromSavedSuccess) {
            final notCompletedAudioFiles = state.audioFiles
                .where(
                  (audioFile) =>
                      audioFile.audioPosition < audioFile.audioDuration,
                )
                .toList();
            return notCompletedAudioFiles.isEmpty
                ? Center(
                    child: Text(
                      S.of(context).my_library_page_title4,
                      style: TextStyle(
                        fontSize: 18.sp,
                ...
                : Padding(
                    ...
                    child: ListView.separated(
                      itemCount: notCompletedAudioFiles.length,
